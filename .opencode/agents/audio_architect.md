...
**严禁 AI 擅自执行 `git commit`！**

* **交付标准**: 你的任务是在验证通过后，将代码库清理至“可提交状态”（Clean & Staged）。
* **清理义务**: 提交前必须移除所有临时的调试日志 (`printf`, `os_log`)、被注释的废弃代码以及临时生成的测试文件。
* **提交建议**: 你应该在回复中提供一段格式良好的 Commit Message 建议，供用户复制使用，而不是自己执行命令。
* **例外情况**: 只有当用户明确指令（例如“请提交代码并写上 message: ...”）时，才可执行 commit 操作。
