...
// 设备初始化
OSStatus
virtual_device_create (VirtualAudioDevice **outDevice)
{
  if (outDevice == NULL)
    {
  ...
  VirtualAudioDevice *device
    = (VirtualAudioDevice *) calloc (1, sizeof (VirtualAudioDevice));
  if (device == NULL)
    {
  ...
  device->deviceUID
    = CFStringCreateWithCString (NULL, "com.ahogek.virtualaudiodriver",
				 kCFStringEncodingUTF8);
  device->deviceName = CFStringCreateWithCString (NULL, "Virtual Audio Driver",
						  kCFStringEncodingUTF8);
  ...
  // 分配音频缓冲区
  device->outputStream.bufferList
    = (AudioBufferList *) calloc (1, sizeof (AudioBufferList)
				       + sizeof (AudioBuffer));
...
// 设备销毁
void
virtual_device_destroy (VirtualAudioDevice *device)
{
  if (device != NULL)
    {
      // 停止设备
...
// 设备启动
OSStatus
virtual_device_start (VirtualAudioDevice *device)
{
  // 参数检查
  if (device == NULL)
    {
...
// 设备停止
OSStatus
virtual_device_stop (VirtualAudioDevice *device)
{
  // 参数检查
  if (device == NULL)
    {
...
// 设置静音状态
OSStatus
virtual_device_set_mute (VirtualAudioDevice *device, Boolean mute)
{
  if (device == NULL)
    {
      return kAudioHardwareIllegalOperationError;
...
// 获取静音状态
OSStatus
virtual_device_get_mute (const VirtualAudioDevice *device, Boolean *outMute)
{
...
// 设置音量
OSStatus
virtual_device_set_volume (VirtualAudioDevice *device, Float32 volume)
{
  if (device == NULL)
    {
      return kAudioHardwareIllegalOperationError;
...
// 获取音量
OSStatus
virtual_device_get_volume (const VirtualAudioDevice *device, Float32 *outVolume)
{
...
void
apply_volume_and_clamp (Float32 *samples, UInt32 sampleCount,
			Float32 volumeScale)
{
...
// 输出处理
OSStatus
virtual_device_process_output (const VirtualAudioDevice *device,
			       AudioBufferList *outputData, UInt32 frameCount)
{
...
// 获取设备状态
OSStatus
virtual_device_get_state (VirtualAudioDevice *device, DeviceState *outState)
{
...
// 获取设备运行状态
OSStatus
virtual_device_is_running (const VirtualAudioDevice *device,
