                                                   ...
                                                   const AudioServerPlugInClientInfo* inClientInfo)
{
    ...
    if (inClientInfo != NULL)
    {
        ...
        {
            CFStringGetCString(inClientInfo->mBundleID, bundleId, sizeof(bundleId), kCFStringEncodingUTF8);
        }
        if (inClientInfo->mBundleID != NULL)
        {
            CFStringGetCString(inClientInfo->mBundleID, name, sizeof(name), kCFStringEncodingUTF8);
        }

        app_volume_driver_add_client(inClientInfo->mClientID, inClientInfo->mProcessID, bundleId, name);
    }
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
    ...
    case kAudioObjectPropertyName:
        ...
        *((CFStringRef*)outData) = (inObjectID == kObjectID_Stream_Input)
...
#pragma unused(inIOCycleInfo, ioSecondaryBuffer)
    ...
    else if (inOperationID == kAudioServerPlugInIOOperationWriteMix)
    {
        ...
        {
            // 应用音量控制 - 2通道32位浮点
            app_volume_driver_apply_volume(inClientID, ioMainBuffer, inIOBufferFrameSize, 2);
        }
    }
...
static OSStatus VirtualAudioDriver_EndIOOperation(AudioServerPlugInDriverRef inDriver,
                                                  AudioObjectID inDeviceObjectID,
                                                  UInt32 inClientID,
                                                  UInt32 inOperationID,
                                                  UInt32 inIOBufferFrameSize,
                                                  const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
    // 这是在 IO 操作结束时调用的。此设备不执行任何操作，因此只需检查参数并返回。
...
#pragma unused(inClientID, inOperationID, inIOBufferFrameSize, inIOCycleInfo)
    ...
    FailWithAction(inDriver != gAudioServerPlugInDriverRef,
                   theAnswer = kAudioHardwareBadObjectError,
                   Done,
                   "VirtualAudioDriver_EndIOOperation: 错误的驱动引用");
    FailWithAction(inDeviceObjectID != kObjectID_Device,
                   theAnswer = kAudioHardwareBadObjectError,
                   Done,
                   "VirtualAudioDriver_EndIOOperation: 错误的设备 ID");
