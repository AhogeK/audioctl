  ...
  Float64 *outSampleTime, UInt64 *outHostTime, UInt64 *outSeed)
{
  ...
  // 计算周期数 (每 512 帧一个周期)
  UInt64 periodCount
    = (UInt64) (elapsedFrames / (Float64) kZeroTimeStampPeriod);
  ...
  *outSampleTime = sampleTime;
  ...
  // 计算对应的主机时间
  Float64 hostTimeFloat
    = (Float64) anchorTime
      + (Float64) (periodCount * kZeroTimeStampPeriod) * hostTicksPerFrame;

  // 【修复】防止 Inf/NaN 和溢出
  if (!isfinite (hostTimeFloat) || hostTimeFloat < 0.0
      || hostTimeFloat > (Float64) UINT64_MAX)
    {
...
abl_ni_clamp_frames (const AudioBufferList *abl, UInt32 ch, UInt32 frames)
{
  ...
  for (UInt32 b = 0; b < ch; b++)
	...
	= (const AudioBuffer *) ((const uint8_t *) &abl->mBuffers[0]
				 + (b * sizeof (AudioBuffer)));
...
static inline void
abl_ni_zero (AudioBufferList *abl, UInt32 ch, UInt32 frames)
{
  UInt32 useCh = (abl->mNumberBuffers < ch) ? abl->mNumberBuffers : ch;
  for (UInt32 b = 0; b < useCh; b++)
    {
      AudioBuffer *buffer = (AudioBuffer *) ((uint8_t *) &abl->mBuffers[0]
					     + (b * sizeof (AudioBuffer)));
  ...
  if (inOperationID == kAudioServerPlugInIOOperationProcessOutput)
    {
      ...
      app_volume_driver_apply_volume_ni (inClientID, leftChannel, rightChannel,
					 frames);
    }
  ...
  if (inObjectID == kObjectID_PlugIn)
	...
	case kAudioDevicePropertyStreams:
	  ...
	  *outDataSize = sizeof (AudioObjectID) * 2;
	  ...
	  case kAudioDevicePropertyStreamConfiguration: {
		...
		// 使用指针访问以避免固定大小数组越界警告
		AudioBuffer *buffer
		  = (AudioBuffer *) ((uint8_t *) &list->mBuffers[0]
				     + (i * sizeof (AudioBuffer)));
	...
	case kAudioStreamPropertyAvailableVirtualFormats:
	  case kAudioStreamPropertyAvailablePhysicalFormats: {
	    ...
	    format->mFormat.mFormatFlags = kAudioFormatFlagIsFloat
					   | kAudioFormatFlagIsPacked
					   | kAudioFormatFlagIsNonInterleaved;
