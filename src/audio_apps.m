...
// 获取进程名称
static char* getProcessName(pid_t pid)
{
...
static void getBundleIdFromPlist(const char* appPath, char* bundleId, size_t maxLen)
{
    bundleId[0] = '\0';

    char plistPath[1024];
    snprintf(plistPath, sizeof(plistPath), "%s/Contents/Info.plist", appPath);
    ...
    {
        if (strstr(line, "CFBundleIdentifier"))
        {
            foundKey = true;
            continue;
        }

        if (foundKey && strstr(line, "<string>"))
        {
...
// 获取应用的 Bundle ID
static void getBundleId(pid_t pid, char* bundleId, size_t maxLen)
{
    bundleId[0] = '\0';
    ...
    char* appSuffix = strstr(pathbuf, ".app/");
    if (!appSuffix)
    {
        size_t len = strlen(pathbuf);
        if (len > 4 && strcmp(pathbuf + len - 4, ".app") == 0)
        {
            getBundleIdFromPlist(pathbuf, bundleId, maxLen);
        }
        return;
    }
