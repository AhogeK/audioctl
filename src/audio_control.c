...
// 获取音频设备属性
static OSStatus
getAudioProperty (AudioDeviceID deviceId, AudioObjectPropertySelector selector,
		  AudioObjectPropertyScope scope, UInt32 element, void *data,
		  UInt32 *dataSize)
{
  AudioObjectPropertyAddress propertyAddress = {selector, scope, element};
  return AudioObjectGetPropertyData (deviceId, &propertyAddress, 0, NULL,
				     dataSize, data);
}

// 检查属性是否可设置
static OSStatus
isPropertySettable (AudioDeviceID deviceId, AudioObjectPropertyScope scope,
		    UInt32 element, Boolean *settable)
{
...
getVolumeInfo (AudioDeviceID deviceId, AudioDeviceType deviceType,
	       Float32 *volume, Boolean *hasVolumeControl)
{
  *volume = 0.0f;
  *hasVolumeControl = false;
  ...
  if (status == noErr && isSettable
      && getAudioProperty (deviceId, kAudioDevicePropertyVolumeScalar, scope, 1,
			   volume, &dataSize)
	   == noErr)
    {
      *hasVolumeControl = true;
  ...
  Float32 volumeDB;
  status
    = getAudioProperty (deviceId, kAudioDevicePropertyVolumeDecibels, scope,
...
// 获取单个范围（输入/输出）的通道数
static UInt32
getChannelCountForScope (AudioDeviceID deviceId, AudioObjectPropertyScope scope)
{
...
// 获取设备通道数量
static void
getChannelCounts (AudioDeviceID deviceId, UInt32 *inputCount,
		  UInt32 *outputCount)
{
  // 初始化输出参数
  *inputCount = 0;
  *outputCount = 0;
  ...
  // 获取输入通道数
  *inputCount
    = getChannelCountForScope (deviceId, kAudioDevicePropertyScopeInput);

  // 获取输出通道数
  *outputCount
    = getChannelCountForScope (deviceId, kAudioDevicePropertyScopeOutput);
}

// 获取基本音频属性
static void
getBasicAudioProperties (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
  UInt32 dataSize;
  ...
  if (getAudioProperty (deviceId, kAudioDevicePropertyNominalSampleRate,
			kAudioObjectPropertyScopeGlobal, 0, &sampleRate,
			&dataSize)
      == noErr)
    {
  ...
  if (getAudioProperty (deviceId, kAudioDevicePropertyStreamFormat,
			kAudioObjectPropertyScopeGlobal, 0, &streamFormat,
			&dataSize)
      == noErr)
    {
...
// 获取设备静音状态
static void
getDeviceMuteState (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
...
// 获取设备传输类型
static void
getDeviceTransportType (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
  UInt32 dataSize = sizeof (UInt32);
  OSStatus status
    = getAudioProperty (deviceId, kAudioDevicePropertyTransportType,
...
// 获取设备状态
static void
getDeviceStatus (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
...
// 检查设备是否在指定范围内运行
static Boolean
isDeviceRunningInScope (AudioDeviceID deviceId, AudioObjectPropertyScope scope)
{
  UInt32 isRunning = 0;
  UInt32 dataSize = sizeof (UInt32);
  AudioObjectPropertyAddress propertyAddress
...
// 检查设备是否是当前默认设备
static Boolean
isDefaultDevice (AudioDeviceID deviceId, AudioObjectPropertyScope scope)
{
...
// 检查设备运行状态
static void
checkDeviceRunningStatus (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
  // 首先检查设备是否活跃
  UInt32 isAlive = 0;
  UInt32 dataSize = sizeof (UInt32);
  ...
  OSStatus status = AudioObjectGetPropertyData (deviceId, &aliveAddress, 0,
						NULL, &dataSize, &isAlive);
  ...
  switch (info->deviceType)
    ...
    case kDeviceTypeInput:
      info->isRunning
	= isDeviceRunningInScope (deviceId, kAudioDevicePropertyScopeInput)
	  || isDefaultDevice (deviceId, kAudioDevicePropertyScopeInput);
      break;
  ...
  if (AudioObjectHasProperty (deviceId, &streamAddress))
    {
      ...
      status = AudioObjectGetPropertyData (deviceId, &streamAddress, 0, NULL,
					   &dataSize, &processingState);
...
OSStatus
getDeviceList (AudioDeviceInfo **devices, UInt32 *deviceCount)
{
  // 获取设备数量
  ...
  // 分配内存并获取设备列表
  const UInt32 numDevices
    = dataSize / sizeof (AudioDeviceID); // 改用不同的变量名
  AudioDeviceID *deviceIds = malloc (dataSize);

  status
    = AudioObjectGetPropertyData (kAudioObjectSystemObject, &propertyAddress, 0,
				  NULL, &dataSize, deviceIds);
...
OSStatus
getDeviceInfo (AudioDeviceID deviceId, AudioDeviceInfo *info)
{
  if (!info)
  ...
  UInt32 dataSize = sizeof (CFStringRef);
  OSStatus status
    = getAudioProperty (deviceId, kAudioDevicePropertyDeviceNameCFString,
			kAudioObjectPropertyScopeGlobal, 0, &deviceName,
			&dataSize);
  if (status != noErr)
    {
      return status; // 返回获取设备名称时的错误状态
    }
  CFStringGetCString (deviceName, info->name, sizeof (info->name),
		      kCFStringEncodingUTF8);
  CFRelease (deviceName);

  // 获取通道信息
  getChannelCounts (deviceId, &info->inputChannelCount,
  ...
  // 获取音量信息
  getVolumeInfo (deviceId, info->deviceType, &info->volume,
		 &info->hasVolumeControl);
...
OSStatus
setDeviceVolume (AudioDeviceID deviceId, Float32 volume)
{
  ...
  status = AudioObjectIsPropertySettable (deviceId, &mutePropertyAddress,
					  &isMuteSettable);
  ...
  for (UInt32 channel = 1; channel <= channelCount; channel++)
    {
      ...
      status = AudioObjectIsPropertySettable (deviceId, &propertyAddress,
					      &isSettable);
      if (status == noErr && isSettable)
	{
	  status = AudioObjectSetPropertyData (deviceId, &propertyAddress, 0,
...
setDeviceActive (AudioDeviceID deviceId)
{
  ...
  status
    = AudioObjectGetPropertyData (kAudioObjectSystemObject, &propertyAddress, 0,
				  NULL, &propertySize, &currentDevice);
...
const char *
getTransportTypeName (const UInt32 transportType)
{
...
const char *
getFormatFlagsDescription (const UInt32 formatFlags)
{
