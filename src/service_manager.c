...
static void write_log(const char* message)
{
    ...
    if (get_log_file_path(log_path, sizeof(log_path)) != 0) return;
...
static pid_t read_pid_file(void)
{
    ...
    if (get_pid_file_path(pid_path, sizeof(pid_path)) != 0) return -1;
...
ServiceStatus service_restart(void)
{
    ...
    if (stop_status != SERVICE_STATUS_SUCCESS && stop_status != SERVICE_STATUS_NOT_RUNNING)
    {
        return stop_status;
    }
