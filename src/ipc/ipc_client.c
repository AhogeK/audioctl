...
int ipc_client_connect(IPCClientContext* ctx)
{
    ...
    // 连接
    if (connect(ctx->fd, (struct sockaddr*)&addr, sizeof(addr)) < 0 && errno != EINPROGRESS && errno != EAGAIN)
    {
        perror("connect");
        close(ctx->fd);
        ctx->fd = -1;
                         ...
                         IPCMessageHeader* response_header, void* response_payload, size_t response_size)
{
    ...
    // 接收响应
    if (ipc_client_recv(ctx, response_header, response_payload, response_size) != 0)
    {
        return -1;
    }
...
int ipc_client_get_volume_fast(IPCClientContext* ctx, pid_t pid, float* volume, bool* muted)
{
    ...
    if (ipc_client_send_sync(ctx, &request, &pid, &response, &vol_response, sizeof(vol_response)) != 0)
    {
        // 请求失败，使用缓存值
        *volume = ctx->cached_volume;
        *muted = ctx->cached_muted;
    ...
    // 检查响应状态
    if (response.command == kIPCCommandResponse && vol_response.status == kIPCStatusOK)
    {
        // 更新缓存
        ctx->cached_pid = pid;
...
int ipc_client_unregister_app(IPCClientContext* ctx, pid_t pid)
{
    ...
    if (ipc_client_send_sync(ctx, &request, &pid, &response, &resp, sizeof(resp)) != 0)
    {
        return -1;
    }
...
int ipc_client_set_app_volume(IPCClientContext* ctx, pid_t pid, float volume)
{
    ...
    if (ipc_client_send_sync(ctx, &request, &req, &response, &resp, sizeof(resp)) != 0)
    {
        return -1;
    }
...
int ipc_client_ping(IPCClientContext* ctx)
{
    ...
    if (ipc_client_send_sync(ctx, &request, NULL, &response, &resp, sizeof(resp)) != 0)
    {
        return -1;
    }
...
bool ipc_client_should_reconnect(IPCClientContext* ctx)
{
    ...
    // 如果连接但长时间无活动（可选：心跳检测）
    uint64_t now = get_timestamp_ms();
    if (ipc_client_is_connected(ctx) && (now - ctx->last_activity) > 30000 && ipc_client_ping(ctx) != 0) // 30秒
    {
        ctx->connected = false;
