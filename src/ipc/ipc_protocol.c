...
int
get_ipc_socket_path (char *path, size_t path_size)
{
  ...
  int written
    = snprintf (path, path_size, "%s/%s", support_dir, IPC_SOCKET_FILENAME);
...
void
ipc_init_header (IPCMessageHeader *header, uint16_t command,
...
bool
ipc_validate_header (const IPCMessageHeader *header)
{
  if (!header)
    return false;
...
const char *
ipc_status_to_string (int32_t status)
{
  switch (status)
    {
    case kIPCStatusOK:
