...
get_home_directory (void)
{
  ...
  if (getlogin_r (login_name, sizeof (login_name)) == 0
      && strlen (login_name) > 0 && strcmp (login_name, "root") != 0)
    {
      struct passwd pwd;
	...
	bufsize = 16384;
      ...
      if (buffer)
	{
	  if (getpwnam_r (login_name, &pwd, buffer, bufsize, &result) == 0
	      && result)
	    {
  ...
  if (sudo_user && strlen (sudo_user) > 0 && strcmp (sudo_user, "root") != 0)
	...
	bufsize = 16384;
      ...
      if (buffer)
	{
	  if (getpwnam_r (sudo_user, &pwd, buffer, bufsize, &result) == 0
	      && result)
	    {
	      static char home_path[PATH_MAX];
	      strncpy (home_path, pwd.pw_dir, sizeof (home_path) - 1);
...
get_support_directory (char *path, size_t path_size)
{
  ...
  int written = snprintf (path, path_size,
			  "%s/Library/Application Support/audioctl", home);
