...
## 2026-02-16 - 渐进式增益方案废弃

* **场景上下文**: 尝试通过渐进式增益提升（从 10% 或 30% 逐步提升到目标音量）来解决 use-virtual 音量突增问题。
* **尝试过的方案**:
    1. 启动时增益 0.1，每帧增加，约 50ms 提升到目标值
    2. 启动时增益为目标音量的 30%，逐步提升
* **发现的问题**:
    - 双重增益叠加：current_gain * output_gain 导致音量计算错误
    - 增益增量计算错误：每帧增量 * frames 导致增长过快或过慢
    - 实际效果：声音先变小，然后变得更小，不符合预期
* **最终决定**:
    - 废弃渐进式增益方案
    - 恢复到简单的固定增益：直接应用 physical_volume 作为增益值
    - 简化代码，移除 target_gain/current_gain/gain_increment 字段
* **结果**: 音量稳定在物理设备水平，无异常波动
