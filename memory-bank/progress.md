...
1. **测试验证**：验证绑定信息功能是否按预期工作（use-virtual 前后虚拟设备状态变化）
2. **代码清理**：确认所有临时调试代码已移除
...
### AI 职责（我）
1. **编译检查**: 运行 `cmake --build` 或 `make` 确保代码能编译（仅编译，不安装）
2. **单测运行**: 确保单元测试通过
3. **日志分析**: 当用户测试出现问题时，负责查看日志分析原因
4. **调试指导**: 提供调试指令和日志查看方法，由开发者执行
5. **架构维护**: 代码审查、架构建议
...
### ⚠️ 关键红线
- **AI 严禁执行安装**: 包括 `./scripts/install.sh install --no-coreaudio-restart`
