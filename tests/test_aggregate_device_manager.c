...
static int
test_aggregate_device_detection ()
{
...
// 测试获取推荐物理设备
static int
test_recommended_physical_device ()
{
...
test_create_destroy ()
{
  ...
  if (!virtual_device_is_installed ())
    {
      printf (
	"SKIP: Virtual device not installed, skipping create/destroy test\n");
      return 0;
    }
...
test_activate_deactivate ()
{
  ...
  if (!virtual_device_is_installed ())
    {
      printf ("SKIP: Virtual device not installed, skipping "
...
int
run_aggregate_device_manager_tests ()
{
