...
test_app_volume_availability ()
{
  ...
  if (!virtual_device_is_installed ())
	...
	{
	  printf (
	    "FAIL: Should not be able to control when device not installed\n");
	  return 1;
	}
    }
  ...
  if (!virtual_device_is_active ())
	...
	{
	  printf (
	    "FAIL: Should not be able to control when device not active\n");
	  return 1;
	}
    }
...
int
run_virtual_device_manager_tests ()
{
