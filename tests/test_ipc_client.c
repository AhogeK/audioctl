...
static int test_ipc_client_cache(void)
{
...
static int test_ipc_client_reconnect(void)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (i >= 5 && should)
        {
            printf("    ❌ FAIL: Should not reconnect when attempts=%d (max reached)\n", i);
            return 1;
        }
    }
...
// 在子进程中启动服务端进行集成测试
static int test_ipc_client_integration(void)
{
