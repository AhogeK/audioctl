...
// 辅助函数：检查浮点数是否相等
static bool
float_equals (Float32 a, Float32 b)
{
...
test_basic_output_processing ()
{
  ...
  Float32 *testData
    = (Float32 *) malloc (frameCount * channelCount * sizeof (Float32));
  if (testData == NULL)
    {
  ...
  AudioBufferList bufferList = {.mNumberBuffers = 1, .mBuffers = {buffer}};
